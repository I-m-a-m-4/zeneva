...
export function BulkImageEditor({
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      try {
        const res = await fetch(
          `${apiBase()}/api/images/search?q=${encodeURIComponent(candidate.product.name)}&limit=6`,
  ...
  const handleSave = async () => {
    ...
    for (const candidate of toSave) {
      ...
      try {
        // Download via proxy
        const proxyUrl = `${apiBase()}/api/images/proxy?url=${encodeURIComponent(image.urls.regular)}&fallback=${encodeURIComponent(image.urls.small)}`;
                      ...
                      {isErr && !isSaving && (
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              const res = await fetch(`${apiBase()}/api/images/search?q=${encodeURIComponent(c.product.name)}&limit=6`);
                              const data = await res.json();
