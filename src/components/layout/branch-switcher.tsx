...
import { Store, Building2, PlusCircle } from 'lucide-react';
...
export function BranchSwitcher({ variant = 'sidebar', className }: BranchSwitcherProps) {
  ...
  const handleBranchChange = (value: string) => {
    if (value === 'manage-branches-action') {
      ...
      <div className={cn("flex items-center", className)}>
        <Select value={activeBranchId} onValueChange={handleBranchChange}>
          ...
          <SelectContent align="start">
            ...
            <SelectItem value="manage-branches-action" className="text-primary font-bold border-t mt-1 pt-2 cursor-pointer flex items-center gap-2">
              <span className="flex items-center gap-1.5"><PlusCircle className="h-3.5 w-3.5" /> + Manage / Add Branches</span>
            </SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      <div className={cn("w-full py-1.5", className)}>
        ...
        <Select value={activeBranchId} onValueChange={handleBranchChange}>
          ...
          <SelectContent align="start">
            ...
            <SelectItem value="manage-branches-action" className="text-primary font-bold border-t mt-1 pt-2 cursor-pointer flex items-center gap-2">
              <span className="flex items-center gap-1.5"><PlusCircle className="h-4 w-4" /> + Manage / Add Branches</span>
            </SelectItem>
          </SelectContent>
        </Select>
      </div>
