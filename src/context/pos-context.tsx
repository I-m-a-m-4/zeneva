...
export function POSProvider({ children }: { children: ReactNode }) {
  ...
  const productsQuery = useMemoFirebase(() => (canFetchSubData ? query(collection(firestore, "products"), where("businessId", "==", businessId), limit(100000)) : null), [canFetchSubData, businessId, firestore]);
  ...
  const receiptsQuery = useMemoFirebase(() => (canFetchSubData ? query(collection(firestore, "receipts"), where("businessId", "==", businessId), limit(100000)) : null), [canFetchSubData, businessId, firestore]);
  const { data: initialReceipts, isLoading: isLoadingReceipts, mutate: mutateReceipts } = useCollection<Receipt>(receiptsQuery);

  const customersQuery = useMemoFirebase(() => (canFetchSubData ? query(collection(firestore, "customers"), where("businessId", "==", businessId), limit(100000)) : null), [canFetchSubData, businessId, firestore]);
